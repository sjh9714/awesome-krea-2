...
def main() -> int:
    ...
    fails = [f for f in d.get("failures", {}).get("entries", [])
             if (HERE / f["image"]).exists()]
    ...
    head = ["# The catalog", "",
            f"All **{len(entries)}** prompts, each with the seed that produced it, and the "
            f"**{len(fails)}** generations that were cut with the reason each one failed.", "",
            "Every image is raw model output. Nothing was retouched, upscaled or cropped.",
    ...
    for f in fails:
        ...
        fail_lines.append(
            f'| <img src="../{f["image"]}" width="150" alt="{f["claim"]}"> '
    ...
    if not multi:
        ...
        ftext = "\n".join(["[← the categories](gallery.md)", ""] + fail_lines)
        (DOCS / "gallery-failures.md").write_text(ftext, encoding="utf-8")
        written.append(("gallery-failures.md", len(ftext)))
    ...
    (DOCS / "gallery-map.json").write_text(
        json.dumps({"multi": multi, "where": where,
                    "failures": "gallery-failures.md" if multi else "gallery.md#the-failures"},
                   indent=2) + "\n", encoding="utf-8")

    for name, n in written:
        print(f"docs/{name}  {n // 1024} KB")
    print(f"{len(entries)} entries across {len(cats)} categories, {len(fails)} failures")
    return 0
